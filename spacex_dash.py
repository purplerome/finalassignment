...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        return px.pie(spacex_df, values='class', names='Launch Site', title='Launch Success Rate For All Sites')
    else:
...
def get_scatter_chart(entered_site, slider):
    ...
    if entered_site == 'ALL':
        return px.scatter(filtered_df2, x='Payload Mass (kg)', y='class', title='Launch Success Rate For All Sites', color='Booster Version Category')
             
    # return the outcomes in pie chart for a selected site
    else:
            filtered_df2 = filtered_df2[filtered_df2['Launch Site'] == entered_site]
            filtered_df2['outcome'] = filtered_df2['class'].apply(lambda x: 'Success' if (x == 1) else 'Failure')
            filtered_df2['counts'] = 1
            return px.scatter(filtered_df2, x='Payload Mass (kg)', y='class', title='Launch Success Rate For '  + entered_site, color='Booster Version Category')
# TASK 2:
